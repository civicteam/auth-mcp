...
export class CLIAuthProvider extends CivicAuthProvider {
	...
	async redirectToAuthorization(authorizationUrl: URL): Promise<void> {
		console.log(
			`Opening authorization URL in browser: ${authorizationUrl.href}`,
		);
	...
	private async startCallbackServer(): Promise<void> {
		return new Promise((resolve, reject) => {
				...
				if (parsedUrl.pathname === "/callback") {
					...
					if (error) {
						...
						if (this.transport) {
							...
							this.authorizationCodeReject?.(
								new Error("No transport registered"),
							);
						}
					} else {
						res.writeHead(400);
			...
			this.callbackServer.listen(this.callbackPort, () => {
				console.log(
					`Callback server listening on http://localhost:${this.callbackPort}`,
				);
				resolve();
			});
