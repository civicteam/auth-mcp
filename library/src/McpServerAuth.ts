...
export class McpServerAuth<TRequest extends IncomingMessage = IncomingMessage> {
     ...
     */
    private async createAuthInfo(token: string | null, payload: JWTPayload | null, request?: TRequest): Promise<ExtendedAuthInfo | null> {
        const inputAuthInfo: ExtendedAuthInfo | null = (token && payload) ? {
            token,
